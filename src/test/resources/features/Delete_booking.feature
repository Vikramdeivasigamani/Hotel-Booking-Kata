...
Feature: Cancel booking
  ...
  Scenario: Unsuccessful cancellation due to invalid token code
    Given a confirmed booking already exists
    When the customer cancels the booking using booking id "<bookingid>" and token code "<token>"
    Then the response status code should be 401
    And the response displays an error message "<errormessage>"
    ...
    Examples:
      ...
      @delete @negative @validation @error
  Scenario: Unsuccessful cancellation due to missing token code
    Given a confirmed booking already exists
    When the customer cancels the booking using booking id "<bookingid>" and token code "<token>"
    Then the response status code should be 401
    And the response displays an error message "<errormessage>"
  ...
  @delete @negative @validation @error @validation
  Scenario: Unsuccessful cancellation of an already cancelled booking
  ...
  @delete @negative @validation @error @validation
  Scenario: Unsuccessful cancellation of an booking after check-in date
    Given a confirmed booking already exists
  ...
  @delete @negative @validation @error @validation
  Scenario: Unsuccessful cancellation of an expired booking
    Given a confirmed booking already exists
